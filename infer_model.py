...
from models.densenet import TripletAttention
from models.densenet import PagFM

class Inference(nn.Module):
    def __init__(self, params=None, draw_map=True):
        ...
        self.channel_att2 = TripletAttention(no_spatial=False)
        """经过cnn后 长宽与原始尺寸比缩小的比例"""
        self.ratio = params['densenet']['ratio']
        self.block = PagFM(684, 228)
    ...
    def forward(self, images, labels, name, is_train=False):
        cnn_features1 = self.encoder(images)
        cnn_features2 = self.channel_att2(cnn_features1)
        cnn_features = self.block(cnn_features2, cnn_features1)
